-- Migration: Create Projects Module (service_catalog, projects, project_tasks)
-- Includes: ENUMs, tables, indexes, triggers, RLS policies, aggregation view
...
-- Agency's catalog of services offered, scoped per workspace
-- ============================================================
...
CREATE INDEX idx_service_catalog_workspace ON public.service_catalog(workspace_id);
CREATE INDEX idx_service_catalog_active    ON public.service_catalog(workspace_id, is_active);

-- ============================================================
-- TABLE: projects
-- A project belongs to a workspace; optionally linked to a client account
-- ============================================================

CREATE TABLE public.projects (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id      UUID NOT NULL REFERENCES public.workspaces(id) ON DELETE CASCADE,
  -- Optional link to a client account (connection with connection_type = 'client_account')
...
-- ============================================================
-- TABLE: project_tasks
-- Tasks within a project with kanban status and billing data
-- workspace_id is denormalized for efficient RLS (avoids join through projects)
-- ============================================================
...
CREATE TABLE public.project_tasks (
  ...
  -- Position within the kanban column for ordered rendering and drag & drop
  position           INTEGER NOT NULL DEFAULT 0,
  service_catalog_id UUID REFERENCES public.service_catalog(id) ON DELETE SET NULL,
  -- hourly_rate overrides the service default at the task level
  hourly_rate        NUMERIC(10, 2),
  estimated_hours    NUMERIC(8, 2),
  actual_hours       NUMERIC(8, 2) NOT NULL DEFAULT 0,
  -- Generated column: always = actual_hours * hourly_rate (STORED = persisted on disk)
  -- Cannot be included in INSERT or UPDATE statements
...
CREATE INDEX idx_project_tasks_workspace ON public.project_tasks(workspace_id);
CREATE INDEX idx_project_tasks_assignee  ON public.project_tasks(assignee_id);
CREATE INDEX idx_project_tasks_status    ON public.project_tasks(project_id, kanban_status);
CREATE INDEX idx_project_tasks_billing   ON public.project_tasks(workspace_id, billing_status);
CREATE INDEX idx_project_tasks_position  ON public.project_tasks(project_id, kanban_status, position);
...
-- Aggregation for billing data; inherits RLS from underlying tables
-- ============================================================

CREATE VIEW public.project_cost_summary AS
SELECT
  p.id                                                                              AS project_id,
  p.workspace_id,
  p.client_account_id,
  ...
  COUNT(CASE WHEN t.kanban_status = 'done' OR t.kanban_status = 'billed' THEN 1 END) AS completed_tasks
...
-- ============================================================
-- RLS: project_tasks
-- workspace_id is denormalized so RLS does NOT need a join through projects
-- ============================================================
